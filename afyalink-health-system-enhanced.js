// AfyaLink e-Referral Health System JavaScript - Enhanced Version
class AfyaLinkSystem {
    ...
    // Admin Functions - Enhanced User Management
    showUserManagement() {
    ...
    generateUserId() {
        const timestamp = Date.now().toString().slice(-6);
        document.getElementById('userId').value = `USR${timestamp}`;
    }
    ...
    addUser() {
        ...
        // Add audit log
        this.addAuditLog('user_created', `Added user ${userData.name} (${userData.id})`);
    }
    ...
    addUserToTable(user) {
        ...
        row.innerHTML = `
            ...
            <td>${user.name}</td>
            <td>${user.email}</td>
            <td><span class="status-badge status-${user.role === 'admin' ? 'accepted' : 'completed'}">${user.role}</span></td>
            <td>${user.department}</td>
            <td><span class="status-badge status-accepted">${user.status}</span></td>
            <td>${user.lastLogin}</td>
            <td>
                <button class="btn btn-secondary" onclick="afyaLink.viewUser('${user.id}')">
                    <i class="fas fa-eye"></i>
                </button>
                <button class="btn btn-secondary" onclick="afyaLink.editUser('${user.id}')">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn btn-secondary" onclick="afyaLink.resetUserPassword('${user.id}')">
    ...
    updateUserStats() {
        const totalUsers = document.querySelectorAll('#usersTable tr').length;
        const activeUsers = Math.floor(totalUsers * 0.9); // Mock data
        const newUsers = Math.floor(totalUsers * 0.1); // Mock data
